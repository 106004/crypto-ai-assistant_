...
from crypto_api import get_coin_from_supabase
...
# 3. 市場資料更新：每 5 分鐘更新 Supabase market_data，讓 LINE Bot 查價時只讀 Supabase。
#
...
def _get_favorite_coin_price(symbol):
    """Use only fresh Supabase market_data for scheduled LINE price pushes."""

    return get_coin_from_supabase(symbol)
...
def send_hourly_favorite_coin_price():
    ...
    for user in users:
        ...
        try:
            ...
            # scheduler 不能因為單次資料查詢失敗就整個停止。
            # 所以這裡只記錄錯誤，然後繼續處理下一位使用者。
            print(f"查詢 {user_id} 的 {favorite_coin.upper()} 價格時發生錯誤，已略過：{error}")
            continue

        if coin_data is None:
            print(f"Supabase 暫時查不到 {favorite_coin.upper()} 新鮮價格，已略過 {user_id}。")
            continue

        if isinstance(coin_data, str):
            print(f"Supabase 暫時沒有可推播資料，已略過 {user_id}：{coin_data}")
            continue
...
def update_market_data_job():
    """排程用：定時更新 Supabase market_data。"""

    # 幣價會一直變，但 LINE Bot 不需要每次查詢都即時打外部 API。
    # 每 5 分鐘更新一次，對一般提醒和查詢已經夠新，也能降低 CoinGecko 限流風險。
    print("[Scheduler] 開始更新 Supabase market_data")

    try:
        collect_market_data()
    except Exception as error:
        # 單次更新失敗不能讓整個 scheduler 停掉；先記錄錯誤，下一輪 5 分鐘後會再試。
        print(f"[Scheduler] Supabase market_data 更新失敗：{error}")
        return

    print("[Scheduler] Supabase market_data 更新完成")
